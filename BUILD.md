...
**生产环境** - 所有文件输出到 `dist/` 目录：
```
dist/
├── main.js          (~740 KB) - 主程序（包含所有依赖和翻译）
├── manifest.json    (~0.5 KB) - 插件清单
└── styles.css       (~18 KB)  - 样式文件
```

**开发环境** - 文件输出到根目录：
```
main.js          - 主程序（未压缩，包含 source map）
```
...
**`.github/workflows/build-zh.yml`** - 自动构建测试
- 每次推送到 master 分支时触发
- 运行完整的构建和代码检查
- 上传构建产物（保留 7 天）

**`.github/workflows/release-zh.yml`** - 自动发布
- 推送 tag（如 `v2.36.1-zh`）时触发
